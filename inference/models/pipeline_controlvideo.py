...
class ControlVideoPipeline(DiffusionPipeline):
    ...
    def get_overlap_slide_window_indices(self, video_length, window_size, window_overlap):
        inter_frame_list = []
    ...
    @torch.no_grad()
    def generate_long_video_slidingwindow_overlap(
        self,
        ...
        **kwargs,
        ...
        inter_frame_list = self.get_overlap_slide_window_indices(video_length, window_size, window_overlap)

        # print(key_frame_indices) # [0, 16, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256, 272, 288, 299]
        print(inter_frame_list)
        print(latents.shape)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                cond_final = torch.zeros_like(latents)
                uncond_final = torch.zeros_like(latents)
                out_count_final = torch.zeros((1, 1, latents.shape[2], 1, 1), device=latents.device) # b c f h w
                ...
                for f_i, frame_ids in enumerate(inter_frame_list):
                    ...
                    inter_noise_pred = self.unet(
                        ...
                        inter_frame=False, # be careful this parameter
                    ).sample
                    # perform guidance
                    if do_classifier_free_guidance:
                        noise_pred_uncond, noise_pred_text = inter_noise_pred[:, :, :].chunk(2) # we don't have key frames

                    cond_final[:, :, input_frame_ids] += noise_pred_text
                    uncond_final[:, :, input_frame_ids] += noise_pred_uncond
                    out_count_final[:, :, input_frame_ids] += 1

                # normalize cond and uncond via division by context usage counts
                # print(out_count_final)
                cond_final /= out_count_final
                uncond_final /= out_count_final
                # perform guidance
                noise_pred = cond_final
                if do_classifier_free_guidance:
                    # noise_pred_uncond, noise_pred_text = inter_noise_pred[:, :, 2:].chunk(2)
                    noise_pred_uncond = uncond_final
                    noise_pred_text = cond_final
                    # print(noise_pred_text.shape, noise_pred_uncond.shape)
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
                # print(noise_pred[:, :, frame_ids].shape)
