...
class Face_detect_crop:
    ...
    def get_kps(self, img, crop_size, max_num=0):
        bboxes, kpss = self.det_model.detect(img,
                                             threshold=self.det_thresh,
                                             max_num=max_num,
                                             metric='default')
    ...
    def smooth_kps(self, kpss):
        kps = np.mean(kpss, axis=0)
        return kps

    def get_smooth_m(self, img, crop_size, kpss):
        '''
        kpss means kps with nearest frame
        '''
        kps = self.smooth_kps(kpss)
        M, _ = face_align.estimate_norm(kps, crop_size, mode = self.mode) 
        align_img = cv2.warpAffine(img, M, (crop_size, crop_size), borderValue=0.0)
