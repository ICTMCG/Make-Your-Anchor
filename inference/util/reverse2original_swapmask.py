...
def encode_segmentation_rgb(segmentation, no_neck=True):
    ...
    face_part_ids = [1, 2, 3, 4, 5, 6, 10, 12, 13, 17] if no_neck else [1, 2, 3, 4, 5, 6, 7, 8, 10, 12, 13, 14, 17] # my method
    mouth_id = 11
    # hair_id = 17
...
def reverse2wholeimage(b_align_crop_tenor_list,swaped_imgs, mats, crop_size, oriimg, logoclass, save_path = '', \
    ...
    for swaped_img, mat ,source_img in zip(swaped_imgs, mats,b_align_crop_tenor_list):
        swapped_for_parsing = swaped_img.unsqueeze(0)
        ...
        if use_mask:
            # print(source_img.shape, swaped_img.shape)
            # print(type(swaped_img))
            # swapped_for_parsing = torch.from_numpy(swaped_img).cuda()
            # swapped_for_parsing = swapped_for_parsing.reshape(1, 3, swaped_img.shape[0], swaped_img.shape[1])
            source_img_norm = norm(swapped_for_parsing)
            ...
            tgt_mask = encode_segmentation_rgb(vis_parsing_anno, no_neck=False)
            # print(tgt_mask.shape)
            # print(tgt_mask[:,:,0].max())
            # cv2.imwrite('tgt_mask.jpg', 255 * tgt_mask[:,:,0])
